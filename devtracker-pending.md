...
### **Uncategorized**
### **docs**
- [x] 🌀 **[EPIC/COMPAT]** `Export-PHWriterMetadata` → `New-PHWriter` pipeline: property names from the
...
- [x] 🌀 **[EPIC/GLOBALCFG]** `$global:__phwriter` config variable: Allow users to set a module-wide
...
- [x] 🌀 **[EPIC/HELPSEARCH]** Param-level help search: add `-HelpParam <string>` to `New-PHWriter` and
...
- [x] 🟡 **[BUG]** `Show-PHTheme`: first preview is static — the `begin {}` block captures `$resolvedLineSpacing`
...
- [x] 🟡 **[FEATURE]** `Show-PHTheme`: add alias `shldc`; export via `phwriter.psm1`.
- [x] 🟡 **[FEATURE]** `New-PHRouter`: allow `Show-PHTheme` and other public cmdlets to be addressable
      as subcommand routes in the default router dispatch table.
- [x] 🟢 **[QOL]** `New-PHWriter` pipeline param set: accept `-InputObject` (pipeline) of type
...
- [x] 🟢 **[QOL]** Global config merging: when `$global:__phwriter.theme_config` is a hashtable,
...
- [x] 🟢 **[QOL]** `-HelpParam` route search: support `Show-PHTheme -HelpParam <name>` to display only
...
- [x] 🟡🌟1️⃣ **docs** - Update api-reference.md to reflect InputObject, HelpParam, shldc, and global config additions
- [x] 🟠🌟1️⃣ **docs** - Create new cmdlet-reference.md page with full examples for all 8 public cmdlets
- [x] 🟢🩹2️⃣ **docs** - Update _data/cmdlets.yml to include shldc alias for Show-PHTheme
- [x] 🟢🌟2️⃣ **docs** - Update nav_order to slot cmdlet-reference.md at nav_order 4 and shift api-reference.md to 5
